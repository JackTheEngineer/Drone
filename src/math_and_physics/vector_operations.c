...
int32_t Vect_i32_read(Vector_t *vect, uint8_t index){
	// Values from 1 - 3 
	return vect->v[index-1];
}

void Vect_i32_write(Vector_t *vect, uint8_t index, int32_t value){
	vect->v[index-1] = value;
}

int32_t Vect_i32_dot(Vector_t *vect_1, Vector_t *vect_2){
	int32_t sum = 0;
	uint8_t i;
	for(i=1; i<=3;i++){
		sum += Vect_i32_read(vect_1, i)*Vect_i32_read(vect_2, i);
	}
	return sum;
}

void Vect_i32_set_all_values_to(Vector_t *vect, int32_t value){
	Vect_i32_write_three_values(vect, value, value, value);
}

void Vect_i32_add_to(Vector_t *sum_vect, Vector_t *vect){
	Vect_i32_add(vect, sum_vect, sum_vect);
}

void Vect_i32_add(Vector_t *vect_1, Vector_t *vect_2, Vector_t *sum_vect){
...
void Vect_i32_write_three_values(Vector_t *vector, int32_t value_1, int32_t value_2, int32_t value_3){
	Vect_i32_write(vector, 1, value_1);
	Vect_i32_write(vector, 2, value_2);
	Vect_i32_write(vector, 3, value_3);
}

void Vect_i32_times_const(Vector_t *vector, int32_t constant, Vector_t* result_vector){
...
void Vect_i32_copy_from_to(Vector_t *vector_from, Vector_t *vector_to){
	uint8_t i;
	for(i=0; i<3;i++){
		vector_to->v[i] = vector_from->v[i];
	}
...
void Vect_i32_cross_multiply(Vector_t *vector_1, Vector_t *vector_2, Vector_t *resultvector){
...
void Vect_i32_sum_up_list_of_vectors(Vector_t vectorlist[], Vector_t *sum_vector, uint32_t listlength){
	uint32_t i;
...
void Vect_i32_set_vectorlist_to_value(Vector_t vectorlist[], uint32_t listlength, int32_t value){
	uint32_t i;
	
	for(i=0; i<listlength; i++){
		Vect_i32_set_all_values_to(&(vectorlist[i]), value);
	}
}

int32_t *Vect_i32_pointer_to_index(Vector_t *vect, uint8_t index){
	return &(vect->v[index-1]);
}
