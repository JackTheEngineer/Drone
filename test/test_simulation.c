...
_STATIC_ void Run_one_simulation_Step(FILE *file, Motorcontrolvalues_t *motorspeeds);
_STATIC_ void Write_dronedata_to_file_in_a_line(FILE *file);
...
_STATIC_ void Simulate_and_write_to_file(FILE *file){
	...
	for(time_counter=0; time_counter < ITERATIONS; time_counter++){
		Run_one_simulation_Step(file, motorspeeds);
	}
}

_STATIC_ void Run_one_simulation_Step(FILE *file, Motorcontrolvalues_t *motorspeeds){
	Write_dronedata_to_file_in_a_line(file);
